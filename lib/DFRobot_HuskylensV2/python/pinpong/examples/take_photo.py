...
class ProtocolThread(threading.Thread):
    ...
    def run(self):
        ...
        while True:
            ...
            ret = self.husky.takePhoto("1280x720")
            logging.info(f"takePhoto 1280x720 ret={ret}")
            time.sleep(5)
            
            ret = self.husky.takePhoto("1920x1080")
            logging.info(f"takePhoto 1920x1080 ret={ret}")
            time.sleep(5)
