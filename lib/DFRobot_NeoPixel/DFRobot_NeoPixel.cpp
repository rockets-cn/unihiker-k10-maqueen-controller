...
static rmt_obj_t* rmt_send_data = NULL;
static bool initRMT = false;
#endif 
...
#define PERIOD_400 ((int)(2.50 * SCALE + 0.5) - (5 * INST))
...
#if defined(CONFIG_IDF_TARGET_ESP32S3)
    if(!initRMT){
        rmt_send_data = Rmt.txInit(pin, 100, 1); // tick = 100ns
        if(rmt_send_data == NULL){
            Serial.println(" rmt_send create error");
            return;
        }
        
        initRMT =true;
    }
    rmt_data_t *rmt_data = NULL;
    rmt_data_t *temp = (rmt_data_t *)realloc(rmt_data, sizeof(rmt_data_t) * numBytes * 8);
    if (temp == NULL) {
        Serial.println(" rmt_data null");
        return;
    } else {
        rmt_data = temp;
    }
    ...
    bool sta = false;
    do{
        sta = Rmt.writeAsync(rmt_send_data, rmt_data, numBytes * 8);
    }while(!sta);
    free(rmt_data);
    delay(10);
#else
